...
for i=1:n1/2
    for j=1:n2
        sun(i,j)=sin((2*j-1)*%pi/(2*n2))*sin((2*i-1)*%pi/n1)*10;
    end
end 
for i=n1/2+1:n1
    for j=1:n2
...
for k = 1:m
    for i=1:n1
        for j=1:n2
            tp(i,j)=p(i,j)*(W+cp*r(i,j))+sun(i,j);
            tr(i,j)=r(i,j)*(F+cr*p(i,j))+water(i,j);
        end
    end
    sp = sum(tp);
    sr = sum(tr);
    ...
    for i=1:n1
        for j=1:n2
            ...
            if p(i,j)<p(i,jl) then
                tp(i,j)=tp(i,j)+0.2*p(i,jl);
            end
             if p(i,j)<p(i,jr) then
                tp(i,j)=tp(i,j)+0.2*p(i,jl);
            end
             if p(i,j)<p(ir,j) then
                tp(i,j)=tp(i,j)+0.2*p(ir,ji);
            end
             if p(i,j)<p(il,j) then
                tp(i,j)=tp(i,j)+0.2*p(il,ji);
            end
